...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /* ============================================================
     7. TOGGLE STATUS KETERSEDIAAN MENU (dari tabel desktop dan mobile)
        Toggle langsung kirim AJAX ke backend untuk update status
  ============================================================ */  
  // A. Listener untuk versi Desktop (Tabel)
  ...
  // B. Listener untuk versi Mobile (Card List)
  document.querySelector(".menu-card-list")?.addEventListener("change", function (e) {
    if (e.target.classList.contains("toggle-status-menu")) {
      eksekusiToggleStatus(e.target);
    }
  });

  // C. Fungsi Eksekusi Utama AJAX ke Backend Laravel
  function eksekusiToggleStatus(checkbox) {
    const menuId = checkbox.dataset.id;
    const namaMenu = checkbox.dataset.nama || `ID ${menuId}`;
    const isTersedia = checkbox.checked ? 1 : 0; // Ubah ke integer sesuai backend Laravel Anda
    const currentToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
    const statusAwal = !checkbox.checked;

    // SINKRONISASI VISUAL: Cari semua toggle dengan data-id yang sama (Desktop & Mobile) dan samakan posisinya
    document.querySelectorAll(`.toggle-status-menu[data-id="${menuId}"]`).forEach(cb => {
      cb.checked = checkbox.checked;
    });

    // Sesuaikan URL dan Method (Ganti POST/PATCH jika route Laravel Anda berbeda)
    fetch(`/admin/menu/${menuId}/toggle-status`, {
        method: "POST", // Kembalikan ke POST sesuai spesifikasi backend menu.js awal Anda
        headers: {
            "X-CSRF-TOKEN": currentToken,
            "Content-Type": "application/json",
            "Accept": "application/json"
    ...
    .then(dataMenu => {
        if (dataMenu.success) {
            ...
            // Jika backend menolak, kembalikan posisi SEMUA toggle bermenu ID ini
            document.querySelectorAll(`.toggle-status-menu[data-id="${menuId}"]`).forEach(cb => {
    ...
    .catch(err => {
        ...
        // Jika koneksi putus, kembalikan posisi SEMUA toggle bermenu ID ini
        document.querySelectorAll(`.toggle-status-menu[data-id="${menuId}"]`).forEach(cb => {
          cb.checked = statusAwal;
        });
        alert("Terjadi kesalahan koneksi atau method HTTP tidak sesuai.");
    });
  }
